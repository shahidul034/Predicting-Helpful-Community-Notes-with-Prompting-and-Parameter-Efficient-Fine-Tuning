...
\begin{document}
	...
	\begin{abstract}
		Twitter/X Community Notes represents a significant shift toward decentralized, crowdsourced fact-checking, where visibility is reserved for notes judged ``helpful'' by a diverse pool of raters. In this study, we investigate the feasibility of predicting note helpfulness directly from text, comparing a range of modern language modeling strategies. We evaluate the performance of \textbf{Gemma-3-4B-IT} and \textbf{Qwen3-4B-Instruct} across zero-shot, few-shot, and chain-of-thought (CoT) prompting, and benchmark these against a supervised \textbf{BERT} baseline and parameter-efficient fine-tuning (\textbf{PEFT}) via \textbf{LoRA}. 
		Our experiments reveal a stark performance hierarchy: while zero-shot prompting fails to outperform the majority class, CoT reasoning improves F1 scores to \textbf{62.50} (Gemma) and \textbf{61.49} (Qwen). However, these prompting strategies are substantially outperformed by PEFT; LoRA-tuned Qwen and Gemma models achieve F1 scores of \textbf{85.90} and \textbf{96.03}, respectively, on a shared 246-example test split. These results demonstrate that while instruction-tuned models possess strong general reasoning, targeted fine-tuning is essential for capturing the nuanced stylistic and evidentiary patterns that drive community trust. All code and fine-tuned models are publicly available at \href{https://github.com/shahidul034/Predicting-Helpful-Community-Notes-with-Prompting-and-Parameter-Efficient-Fine-Tuning}{GitHub} and Hugging Face (\href{https://huggingface.co/shahidul034/gemma-3-4b-it-CommunityNotesEffectiveness}{Gemma} \& \href{https://huggingface.co/shahidul034/qwen3-4b-instruct-2507-CommunityNotesEffectiveness}{Qwen}).
	\end{abstract}
	...
	We use the public Community Notes data released by Twitter/X.\footnote{\begin{minipage}{\columnwidth}\small\url{https://communitynotes.twitter.com/guide/en/under-the-hood/download-data}\end{minipage}} Following the original project plan, we treat the task as binary classification: predict whether a note is \textsc{currently\_rated\_helpful} or not, while excluding notes labeled \textsc{needs\_more\_ratings}. The final dataset comprises 1,000 notes covering January--December 2024, split temporally into 754 training samples (January--September 2024) and 246 test samples (October--December 2024). All experiments---prompting, BERT fine-tuning, and LoRA fine-tuning---use the same train/test split for consistent comparison.
	
	\noindent\textbf{Class Distribution.} The dataset is nearly balanced: the training set contains 49.7\% Helpful (375) and 50.3\% Not Helpful (379) notes, while the test set contains 54.5\% Helpful (134) and 45.5\% Not Helpful (112) notes. The 4.7-percentage-point difference is negligible, ensuring that our F1-score comparisons are not distorted by class imbalance.
	
	\noindent\textbf{Linguistic Profile.} Community Notes are compact, averaging 31.4 words (SD = 10.2, median = 33). Helpful notes tend to be slightly longer (mean = 32.7 words) than Not Helpful notes (mean = 30.0 words), reflecting the additional detail and structure that effective fact-checking typically requires. The most frequent unigrams are shared across both classes (``claim,'' ``data,'' ``about,'') but their combinatorial patterns differ systematically. Helpful notes are characterized by authoritative, evidence-citing bigrams such as `not consistent with' and `publicly available data,' whereas Not Helpful notes feature hedging phrases such as `hard to say' and `for certain.' Readability, measured by the Flesch--Kincaid grade level, is similar across classes (Helpful: 11.2; Not Helpful: 11.1), indicating that complexity alone does not distinguish effective notes.
	
	\noindent\textbf{Train-Test Consistency.} The temporal split guarantees zero overlap: no note ID appears in both splits. Statistical properties are consistent across splits: word count (Train mean = 31.4, Test mean = 31.7), readability (Train = 11.2, Test = 11.0), and average ratings per note (Train = 50.1, Test = 50.2).
	...
	\begin{table}[t]
		...
		\begin{tabular}{lcc}
			\toprule
			\textbf{Statistic} & \textbf{Train (754)} & \textbf{Test (246)} \\
			\midrule
			Helpful (\%) & 49.7\% & 54.5\% \\
			...
			\bottomrule
		\end{tabular}
		\caption{Dataset statistics and train-test split consistency. The splits are statistically similar across all measured dimensions, confirming a fair comparison setting.}
